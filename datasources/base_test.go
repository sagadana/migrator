...
func (m *MockErrorDatasource) Actions() *DatasourceActionResult {
...
func (m *MockErrorDatasource) Count(ctx *context.Context, request *DatasourceFetchRequest) uint64 {
	return 5 // Report that there's data
}

func (m *MockErrorDatasource) Fetch(ctx *context.Context, request *DatasourceFetchRequest) DatasourceFetchResult {
...
func (m *MockErrorDatasource) Push(ctx *context.Context, request *DatasourcePushRequest) (DatasourcePushCount, error) {
	return DatasourcePushCount{}, fmt.Errorf("simulated push error")
}

func (m *MockErrorDatasource) Watch(ctx *context.Context, request *DatasourceStreamRequest) <-chan DatasourceStreamResult {
	ch := make(chan DatasourceStreamResult)
	close(ch)
	return ch
}
...
func (m *MockErrorDatasource) Import(ctx *context.Context, request DatasourceImportRequest) error {
	return fmt.Errorf("simulated import error")
}

func (m *MockErrorDatasource) Export(ctx *context.Context, request DatasourceExportRequest) error {
...
func compareBatches(got, expected []DatasourcePushRequest) bool {
...
func compareSingleBatch(got, expected DatasourcePushRequest) bool {
...
// runDatasourceTest runs a suite of tests against the provided Datasource implementation.
func runDatasourceTest(testCtx context.Context, t *testing.T, td TestDatasource) {
	...
	t.Run(td.id, func(t *testing.T) {
		...
		actions := td.source.Actions()

		// --------------------------------------------------------------------------
		// Test Basic Read / Write actions
		// --------------------------------------------------------------------------
		if actions.Read && actions.Write {
			t.Run("Test_CRUD", func(t *testing.T) {
				...
				if got := td.source.Count(&ctx, &DatasourceFetchRequest{}); got != 0 {
					t.Fatalf("⛔️ expected empty count=0, got %d", got)
				}

				// 2) Test Push: inserts only
				pushReq := &DatasourcePushRequest{
				...
				if got := td.source.Count(&ctx, &DatasourceFetchRequest{}); got != 2 {
					t.Errorf("❌ expected count=%d, got %d", uint64(len(pushReq.Inserts)), got)
				}

				// 4) Test Fetch: no filters
				fetchRes := td.source.Fetch(&ctx, &DatasourceFetchRequest{Size: 0, Offset: 0})
				if fetchRes.Err != nil {
				...
				// 5) Test Fetch: by IDs
				fetchRes2 := td.source.Fetch(&ctx, &DatasourceFetchRequest{IDs: []string{"b"}})
				...
				fetchRes3 := td.source.Fetch(&ctx, &DatasourceFetchRequest{IDs: []string{"a"}})
				...
				// 8) Test Push: deletes
				pushDel := &DatasourcePushRequest{Deletes: []string{"a", "x"}}
				...
				if got := td.source.Count(&ctx, &DatasourceFetchRequest{}); got != 1 { // a,b,x - a,x = b(1)
			...
			t.Run("Test_Concurrent_CRUD", func(t *testing.T) {
				...
				if count := td.source.Count(&ctx, &DatasourceFetchRequest{}); count != uint64(numOps) {
				...
				// Verify updates
				res := td.source.Fetch(&ctx, &DatasourceFetchRequest{})
				...
				// Verify final count
				if count := td.source.Count(&ctx, &DatasourceFetchRequest{}); count != 0 {
		...
		// --------------------------------------------------------------------------
		// Test Stream / Watch if supported
		// --------------------------------------------------------------------------
		if actions.Stream {
			t.Run("Test_Watch", func(t *testing.T) {
				...
				streamReq := &DatasourceStreamRequest{BatchSize: uint64(evCnt), BatchWindowSeconds: batchWin}
			...
			t.Run("Test_Watch_Multiple_Workers", func(t *testing.T) {
				...
				allEvents := make([][]DatasourceStreamResult, numWorkers)
				mu := new(sync.Mutex)
				...
				for i := range numWorkers {
					...
					go func(workerID int) {
						...
						streamReq := &DatasourceStreamRequest{
							BatchSize:          3,
						...
						count := 0
						events := make([]DatasourceStreamResult, 0)
		...
		// --------------------------------------------------------------------------
		// Test Reads (Fetch, Count, Import, Sort, Filters) if supported
		// --------------------------------------------------------------------------
		if actions.Read {
			t.Run("Test_Fetch", func(t *testing.T) {
				...
				// Get current count
				currentCount := td.source.Count(&ctx, &DatasourceFetchRequest{})
				...
				fetchTests := []fetchTest{
					{
						...
						req:         &DatasourceFetchRequest{Size: 1, Offset: 0},
						expectLen:   1,
						expectStart: 0,
						expectEnd:   1,
					},
					{
						...
						req:         &DatasourceFetchRequest{Size: currentCount + 5, Offset: currentCount + 2},
						expectLen:   0,
						expectStart: 0,
						expectEnd:   0,
					},
					...
					},
					{
						...
						req:         &DatasourceFetchRequest{Size: 1, Offset: currentCount},
						expectLen:   0,
						expectStart: 0,
						expectEnd:   0,
					},
					{
						...
						req:         &DatasourceFetchRequest{Size: 0, Offset: 0},
			...
			t.Run("Test_Count", func(t *testing.T) {
				...
				tests := []struct {
					...
					{
						name:   "Count_SingleID",
						req:    &DatasourceFetchRequest{IDs: []string{"a"}},
						expect: 1,
					},
					{
						name:   "Count_MultipleIDs",
						req:    &DatasourceFetchRequest{IDs: []string{"a", "b", "c"}},
						expect: 3,
					},
					{
						name:   "Count_NonExistingID",
						req:    &DatasourceFetchRequest{IDs: []string{"z"}},
						expect: 0,
					},
					{
						name:   "Count_SizeLessThanTotal",
						req:    &DatasourceFetchRequest{Size: 2},
						expect: 2,
					},
					{
						name:   "Count_SizeGreaterThanTotal",
						req:    &DatasourceFetchRequest{Size: total + 5},
						expect: total,
					},
					{
						name:   "Count_OffsetWithinRange",
						req:    &DatasourceFetchRequest{Offset: 2},
						expect: total - 2,
					},
					{
						name:   "Count_OffsetEqualsTotal",
						req:    &DatasourceFetchRequest{Offset: total},
						expect: 0,
					},
					{
						name:   "Count_OffsetGreaterThanTotal",
						req:    &DatasourceFetchRequest{Offset: total + 1},
						expect: 0,
					},
					{
						name:   "Count_SizeAndOffsetWithinRange",
						req:    &DatasourceFetchRequest{Size: 2, Offset: 1},
						expect: 2,
					},
					{
						name:   "Count_SizeAndOffsetExceedingTotal",
						req:    &DatasourceFetchRequest{Size: 10, Offset: total - 1},
						expect: 1,
					},
					{
					...
					},
					{
						...
						req:    &DatasourceFetchRequest{Size: 0, Offset: 2},
						expect: total - 2,
					},
					{
						name:   "Count_IDsWithOffset",
						req:    &DatasourceFetchRequest{IDs: []string{"a", "b", "c"}, Offset: 1},
						expect: 2,
					},
					{
						name:   "Count_IDsWithSize",
						req:    &DatasourceFetchRequest{IDs: []string{"a", "b", "c"}, Size: 2},
						expect: 2,
					},
					{
						name:   "Count_IDsWithSizeAndOffset",
						req:    &DatasourceFetchRequest{IDs: []string{"a", "b", "c"}, Size: 1, Offset: 2},
						expect: 1,
					},
					{
						name:   "Count_IDsWithSizeAndOffsetExceeding",
						req:    &DatasourceFetchRequest{IDs: []string{"a", "b", "c"}, Size: 5, Offset: 2},
						expect: 1,
					},
				}
			...
			t.Run("Test_Export_CSV", func(t *testing.T) {
				...
				// Call SaveCSV
				err = SaveCSV(&ctx, td.source, csvPath, 2)
				if err != nil {
...
func TestStreamChanges(t *testing.T) {
	...
	tests := []struct {
		name                   string
		events                 []DatasourcePushRequest
		request                DatasourceStreamRequest
		sendEventsCloseWatcher bool
		cancelContextAfterSend bool
		expectedBatches        []DatasourcePushRequest
	}{
		{
		...
		},
		{
			...
			events:                 nil,
			request:                DatasourceStreamRequest{BatchSize: 1, BatchWindowSeconds: 1},
			sendEventsCloseWatcher: true,
			cancelContextAfterSend: false,
			expectedBatches:        []DatasourcePushRequest{},
		},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			results := StreamChanges(&ctx, "test", watcher, &tc.request)
			...
			got := make([]DatasourcePushRequest, 0)
...
func TestSaveCSV(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ds := NewMemoryDatasource("test-save-csv", "id")
			defer ds.Close(&ctx)
			...
			// Call SaveCSV
			err := SaveCSV(&ctx, ds, csvPath, tt.batchSize)
			if err != nil {
				t.Fatalf("⛔️ SaveCSV failed: %v", err)
			}
...
func TestSaveCSV_DatasourceErrors(t *testing.T) {
	...
	t.Run("invalid file path", func(t *testing.T) {
		...
		err = SaveCSV(&ctx, ds, "/invalid/path/file.csv", 10)
		if err == nil {
...
func TestLoadCSV(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ds := NewMemoryDatasource("test-load-csv", "id")
			defer ds.Close(&ctx)

			// Call LoadCSV
			err = LoadCSV(&ctx, ds, csvPath, tt.batchSize)
			if err != nil {
...
func TestLoadCSV_ErrorCases(t *testing.T) {
	...
	t.Run("non-existent file", func(t *testing.T) {
		...
		err := LoadCSV(&ctx, ds, "/non/existent/file.csv", 10)
		if err == nil {
	...
	t.Run("invalid CSV format", func(t *testing.T) {
		...
		ds := NewMemoryDatasource("test-invalid", "id")
		defer ds.Close(&ctx)

		err = LoadCSV(&ctx, ds, csvPath, 5)
...
func TestParseGormFieldValue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := ParseGormFieldValue(tt.field, tt.input)
...
func TestParseGormFieldValue_EdgeCases(t *testing.T) {
	...
	t.Run("float precision edge cases", func(t *testing.T) {
		...
		result, err := ParseGormFieldValue(field, "1.7976931348623157e+308")
		if err != nil {
		...
		result, err = ParseGormFieldValue(field, "2.2250738585072014e-308")
		if err != nil {
	...
	t.Run("int overflow edge cases", func(t *testing.T) {
		...
		result, err := ParseGormFieldValue(field, maxInt64)
		if err != nil {
		...
		result, err = ParseGormFieldValue(field, minInt64)
		if err != nil {
		...
		_, err = ParseGormFieldValue(field, overflow)
		if err == nil {
	...
	t.Run("uint overflow edge cases", func(t *testing.T) {
		...
		result, err := ParseGormFieldValue(field, maxUint64)
		if err != nil {
	...
	t.Run("complex JSON in string field", func(t *testing.T) {
		...
		result, err := ParseGormFieldValue(field, complexJSON)
		if err != nil {
	...
	t.Run("empty string inputs", func(t *testing.T) {
		...
		for _, tt := range tests {
			...
			result, err := ParseGormFieldValue(field, "")
