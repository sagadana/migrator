...
// getStatesTempBasePath returns a temporary base path for file-based state stores.
...
// runStateStoreTests runs a suite of tests against the provided Store implementation.
func runStateStoreTests(testCtx context.Context, t *testing.T, st TestState) {
	...
	t.Run(st.id, func(t *testing.T) {
		...
		t.Run("StoreAndLoad", func(t *testing.T) {
			...
			original := State{
				MigrationStatus:    MigrationStatusInProgress,
				...
				ReplicationStatus:      ReplicationStatusStreaming,
		...
		t.Run("CloseAndCheckOps", func(t *testing.T) {
			...
			if err := st.store.Store(&ctx, "x", original); err != ErrStoreClosed {
				t.Errorf("❌ expected Store after Close to return ErrStoreClosed, got %v", err)
			}
			if _, ok := st.store.Load(&ctx, "a"); ok {
				t.Error("❌ expected Load after Close to return ok=false")
			}
			if err := st.store.Delete(&ctx, "b"); err != ErrStoreClosed {
				t.Errorf("❌ expected Delete after Close to return ErrStoreClosed, got %v", err)
			}
			if err := st.store.Clear(&ctx); err != ErrStoreClosed {
				t.Errorf("❌ expected Clear after Close to return ErrStoreClosed, got %v", err)
			}
		})
	})
}
...
func TestStateJSONRoundTrip(t *testing.T) {
	...
	original := State{
		...
		ReplicationStatus:      ReplicationStatusPaused,
